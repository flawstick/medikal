...
import type { CreateMissionRequest, CreateDriverRequest, CreateCarRequest } from "@/lib/types"
...
export const validators = {
  ...
  plateNumber: (value: string): boolean => {
    ...
    const plateRegex = /^[\u05D0-\u05EA\d]{2,3}[-\s]?[\d]{2,3}[-\s]?[\d]{2,3}$|^\d{2,3}[-\s]?\d{2,3}[-\s]?\d{2,3}$/
    return plateRegex.test(value.trim())
  }
}

// Mission validation
export function validateMission(data: Partial<CreateMissionRequest>): ValidationResult {
  ...
  if (data.metadata?.phone_number && !validators.phone(data.metadata.phone_number)) {
    errors.push({ field: 'metadata.phone_number', message: messages.phone })
  }

  if (data.metadata?.client_name && !validators.minLength(2)(data.metadata.client_name)) {
    errors.push({ field: 'metadata.client_name', message: messages.minLength(2) })
  }

  // Certificates validation
  if (data.certificates && Array.isArray(data.certificates)) {
    data.certificates.forEach((cert, index) => {
      if (cert.packages_count !== undefined && !validators.positiveNumber(cert.packages_count)) {
        errors.push({ 
...
// Driver validation
export function validateDriver(data: Partial<CreateDriverRequest>): ValidationResult {
  const errors: ValidationError[] = []
...
export function validateCar(data: Partial<CreateCarRequest>): ValidationResult {
  ...
  } else if (!validators.plateNumber(data.plate_number)) {
