...
import { Upload, CheckCircle, AlertCircle, FileSpreadsheet, FileText, Edit, X, Plus, Trash2, Package, CalendarIcon } from "lucide-react"
...
export function UploadDeliveryForm() {
  ...
  const [parsedMissions, setParsedMissions] = useState<any[]>([])
  ...
  useEffect(() => {
    ...
    const loadGoogleMapsScript = () => {
      ...
      if (!apiKey) {
        console.warn('Google Maps API key not found. Autocomplete will not be available.')
        return
      }
  ...
  const initializeAutocomplete = () => {
    ...
    if (addressInputRef.current && !addressAutocomplete) {
      const addressAuto = new window.google.maps.places.Autocomplete(addressInputRef.current, {
        types: ['address'],
    ...
    if (cityInputRef.current && !cityAutocomplete) {
      const cityAuto = new window.google.maps.places.Autocomplete(cityInputRef.current, {
        types: ['(cities)'],
        componentRestrictions: { country: 'il' },
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.type.trim()) {
      toast({
        title: "שגיאה",
        description: "סוג משימה הוא שדה חובה",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      toast({
        title: "שגיאה",
        description: error instanceof Error ? error.message : "שגיאה ביצירת המשימה",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const parseExcelFile = async (file: File) => {
    ...
    const missions = (jsonData as any[]).slice(1).map((row: any[]) => ({
      type: row[0] || 'משלוח',
      subtype: row[1] || '',
      address: row[2] || '',
      city: row[3] || '',
  ...
  const parseCsvFile = async (file: File) => {
    ...
    const missions = lines.slice(1).map(line => {
        ...
        parsedMissions.map(async (mission) => {
          const response = await fetch("/api/orders", {
            ...
            body: JSON.stringify({
              ...
              certificates: mission.certificates.length > 0 ? mission.certificates : null,
            }),
          })
          return response.ok
        })
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      <Card className={`flex-1 flex flex-col shadow-sm w-[1200px] mx-auto h-full transition-all duration-300`}>
                ...
                <>
                  ...
                  <div className="bg-card border rounded-lg p-4">
                    ...
                    <p className="text-xs text-muted-foreground mt-2 text-right">
                      השורה הראשונה צריכה להכיל את כותרות העמודות. סוג משימה, כתובת ועיר הם שדות חובה. תעודות יתווספו באופן ידני
                    </p>
                  </div>
                </>
              ) : (
                      ...
                      <div key={index} className="border rounded-lg p-3 bg-background">
                          ...
                          <div className="space-y-1">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                updateCertificate(index, 'packages_count', value ? parseInt(value) : 0)
                              }}
                        ...
                        <div className="mt-3 space-y-1">
                          ...
                          <Textarea
                            ...
                            onChange={(e) => updateCertificate(index, 'notes', e.target.value)}
